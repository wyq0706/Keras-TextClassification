...
def train(hyper_parameters=None, rate=1.0):
    if not hyper_parameters:
        hyper_parameters = {
            'len_max': 160,  # 句子最大长度, 固定推荐20-50, bert越长会越慢, 占用空间也会变大, 本地win10-4G设为20就好, 过大小心OOM
            ...
            'model': {'label': 19,  # 类别数
                      ...
                      'epochs': 50,  # 训练最大轮次
                      'patience': 3,  # 早停,2-3就好
                      'lr': 4e-3,  # 学习率, bert取5e-5, 其他取1e-3, 对训练会有比较大的影响, 如果准确率一直上不去,可以考虑调这个参数
                      'l2': 1e-9,  # l2正则化
...
def evaluate(path_hyper_parameter=path_hyper_parameters, rate=1.0):
    ...
    for item in dict_all:
        ...
        tb.add_row([item, recall, precision, dict_all[item]['TP'], dict_all[item]['FP'], dict_all[item]['TN'],
                    dict_all[item]['FN']])
    print(tb)
    # log
