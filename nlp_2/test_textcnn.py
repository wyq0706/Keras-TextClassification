...
def append_log(hyper_parameters, acc, not_none_acc, threshold):
    ...
    wt = ["TextCNN\n",str(datetime.datetime.now()), str(hyper_parameters), "\nacc: " + str(acc),
...
def checkDensity(predict, truth):
    density=[0,0,0,0]
    for item in predict:
        if item[0][0]=='低':
            density[0]=1
    ...
    for item in truth:
        ...
        if item[0:3]=='无密度':
            density_real[3]=1
    for index, item in enumerate(density):
        if item!=density_real[index]:
            return False
    return True
